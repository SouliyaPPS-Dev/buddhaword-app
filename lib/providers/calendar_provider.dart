...
class CalendarProvider with ChangeNotifier {
  ...
  Future<void> fetchData() async {
    ...
    String? cachedData = prefs.getString('slideLocalData');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _data = values;
        prefs.setString('slideLocalData', json.encode(_data));
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  List<dynamic>? getEventById(String id) {
    // In calendar sheet, id is likely row index or first column
    // Let's assume row index for now if not explicitly provided
    try {
